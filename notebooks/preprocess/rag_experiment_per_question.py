...
CONFIG = {
    ...
    "top_k": 15,
    "max_tokens": 2000,             # non gpt-5 (legacy)
    "max_completion_tokens": 2000,  # gpt-5 (Responses API)
    "temperature": 0.1,             # non gpt-5 (legacy)
    "alpha": 0.7,                   # hybrid weight for vector score
    "max_context_chars": 4000,      # context hard cap (chars)

    # NEW: per-question mode controls
    "per_question_retrieve": True,  # True면 질문마다 retrieve 따로 수행
    "per_question_generate": True,  # True면 질문마다 generate 1회 수행(=프롬프트 1회)
    "save_context_per_question": False,  # metrics에 context_map 저장(파일 커짐)

    # NEW: optional retry for rate-limits / transient failures
    "gen_retry": {
        "enabled": False,           # 필요하면 True로
        "max_retries": 6,
        "initial_delay": 1.0,
        "max_delay": 60.0,
        "jitter": True,
...
def eval_gen(pred: str, gold: Optional[str], threshold: int = 80) -> Dict[str, float]:
    ...
    # 정책:
    # - NOT_FOUND: 정상 출력(근거 부재) -> fill=1
    # - GEN_FAIL: 생성 실패 -> fill=0
    pred_l = pred.lower()
    if pred_l in {"gen_fail"}:
        fill = 0.0
    elif pred_l in {"", "없음"}:
        fill = 0.0
    else:
        # NOT_FOUND 포함, 그 외 정상 텍스트 포함
        fill = 1.0
    ...
    gold = str(gold).strip()
    sim = fuzz.token_set_ratio(pred, gold)  # 문자열 유사도 계산 [web:31]
...
# Generators
# -------------------------
def _retry_sleep(attempt: int, initial_delay: float, exponential_base: float = 2.0,
                 jitter: bool = True, max_delay: float = 60.0) -> None:
    delay = initial_delay * (exponential_base ** max(0, attempt - 1))
    if jitter:
...
class OpenAIGenerator(BaseGenerator):
    ...
    def generate(self, queries: List[Tuple[str, str]], context: str) -> Dict[str, str]:
        ...
        retry_cfg = CONFIG.get("gen_retry", {}) or {}
        ...
        while True:
            ...
            try:
                ...
                # 선택: 레이트리밋/일시 오류 대응(지수 백오프)
                # OpenAI는 rate limit 회피를 위해 exponential backoff를 권장한다. [page:1]
...
class RAGExperiment:
    ...
    def _build_context(self, chunks: List[str], idxs: List[int]) -> str:
        return "".join(chunks[int(i)] for i in idxs if 0 <= int(i) < len(chunks))
    ...
    def run_single_doc_metrics(
        ...
        # 2) chunk -> index (문서당 1회)
        chunks = self.chunker.chunk(doc_path)
        index = self.retriever.build_index(chunks)

        per_q_retrieve = bool(CONFIG.get("per_question_retrieve", True))
        per_q_generate = bool(CONFIG.get("per_question_generate", True))
        save_ctx_map = bool(CONFIG.get("save_context_per_question", False))
        ...
        if per_q_generate:
            # 질문당: retrieve -> context -> generate 1회
            for t, q in queries:
                if per_q_retrieve:
                    idxs = self.retriever.retrieve(index, [q], top_k=top_k)
                else:
                    idxs = self.retriever.retrieve(index, q_texts, top_k=top_k)

                context = self._build_context(chunks, idxs)

                if save_ctx_map:
                    ctx_map[t] = context
                idxs_map[t] = [int(x) for x in idxs]

                one_pred = self.generator.generate([(t, q)], context)
                pred_map[t] = one_pred.get(t, "NOT_FOUND")
        else:
            # 기존 방식(한 번에 generate)
            idxs = self.retriever.retrieve(index, q_texts, top_k=top_k)
            context = self._build_context(chunks, idxs)
        ...
        # 6) retrieval eval
        # per-question retrieve를 켰으면, 질문별 idxs로 평가하는 게 더 일관적
        ...
        for _, row in qdf.iterrows():
            ...
            if per_q_generate and per_q_retrieve and field in idxs_map:
                use_idxs = idxs_map[field]
            else:
                # fallback: 전체 질문 합친 retrieve 1회(또는 질문 목록이 gold field와 1:1이 아닐 때)
                use_idxs = self.retriever.retrieve(index, q_texts, top_k=top_k)

            r_list.append(eval_retrieval_by_anchor(chunks, use_idxs, anchors))
        ...
        metrics: Dict[str, Any] = {
            ...
            # outputs
            "pred_map": pred_map,
            "idxs_map": idxs_map if (per_q_generate and per_q_retrieve) else None,
            "ctx_map": ctx_map if save_ctx_map else None,
        ...
        # cleanup
        del chunks, index, answers, qdf, r_list, g_list, queries, q_texts, GOLD_ANCHOR, preds, pred_map, ctx_map, idxs_map
        gc.collect()
        return metrics